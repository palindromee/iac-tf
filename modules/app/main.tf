# Application Layer Terraform Module
# Using terraform-aws-modules/autoscaling/aws
...
resource "aws_security_group_rule" "app_ingress" {
  ...
  # Only set one source type at a time
...
resource "aws_security_group_rule" "app_egress" {
  ...
  # Only set one destination type at a time
  cidr_blocks = each.value.self == false ? each.value.cidr_blocks : null
  self        = each.value.self == true ? true : null
}

# Auto Scaling Group using terraform-aws-modules
module "autoscaling" {
  ...
  # User data script for Apache setup and health check
